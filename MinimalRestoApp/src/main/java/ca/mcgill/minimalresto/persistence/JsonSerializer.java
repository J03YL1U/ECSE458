...
public class JsonSerializer {
  ...
  public void serialize(Object object, String filename) {
    try {
      String json = formatJsonString(xStream.toXML(object)) + "\n";
  ...
  private static String formatJsonString(final String json) {
    ...
    for (int i = 0, indent = 0; i < chars.length; i++) {
      ...
      if (!beginQuotes) {
        ...
        if (c == '{' || c == '[') {
        	result += c + newline + String.format("%" + (indent += INDENT_WIDTH) + "s", "");
        }
        else if (c == '}' || c == ']') {
        	result += newline + ((indent -= INDENT_WIDTH) > 0 ? String.format("%" + indent + "s", "") : "") + c;
        }
        else if (c == ':') {
        	result += c + " ";
        }
        else if (c == ',') {
        	result += c + newline + (indent > 0 ? String.format("%" + indent + "s", "") : "");
        }
